...
class CourseController {
  static async index (request, reply) {
    const courses = await Course.query().select('id', 'name', 'description', 'education_level')/*.where('education_level', request.user.education_level)*/
    reply.send(courses)
  }
  
  static async courseOfEducationLevel (request, reply) {
    const courses = await Course.query()/*.where('education_level', request.user.education_level)*/.select('id', 'name', 'description', 'education_level')
    reply.send(courses)
  }
  ...
  static async courseChapters (request, reply) {
    ...
    const course = await Course.query().where('id', request.params.courses_id).first()
    if(!course) {
      reply.code(404).send({
        statusCode: 404,
        error: "Not Found",
